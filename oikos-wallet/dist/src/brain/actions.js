 ...
 */
export async function executeActions(actions, services) {
    const results = [];
    for (const action of actions) {
        ...
        try {
            ...
            if (dataObj && typeof dataObj['error'] === 'string') {
                results.push({ tool: action.tool, success: false, error: dataObj['error'] });
            }
            else {
 ...
 */
export async function processActions(reply, services) {
