...
describe('IPCListener DeFi: valid swap messages', () => {
    it('parses valid propose_swap', () => {
        const messages = [];
        const malformed = [];
        const listener = new IPCListener((req) => messages.push(req), (line) => malformed.push(line));
...
describe('IPCListener DeFi: valid yield messages', () => {
    ...
    it('parses valid propose_yield withdraw', () => {
        const messages = [];
        const listener = new IPCListener((req) => messages.push(req), () => { });
        listener.feed(JSON.stringify({
            id: 'yield-2',
            type: 'propose_yield',
            payload: {
...
describe('IPCListener DeFi: malformed swap messages', () => {
    it('drops swap with missing toSymbol', () => {
        const messages = [];
        const malformed = [];
        const listener = new IPCListener((req) => messages.push(req), (line, error) => malformed.push({ line, error }));
        listener.feed(JSON.stringify({
            id: 'bad-swap-1',
            type: 'propose_swap',
            payload: {
...
//# sourceMappingURL=listener-defi.test.js.map